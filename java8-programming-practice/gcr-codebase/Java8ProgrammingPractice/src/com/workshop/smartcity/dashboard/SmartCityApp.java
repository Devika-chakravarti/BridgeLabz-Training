...
public class SmartCityApp {
	public static void main(String[] args) {
		...
		System.out
				.println("============================ Smart City Transport Dashboard ============================\n");
		System.out.println("Current Active Services:");
		services.forEach(TransportService::printServiceDetails);

		System.out.println(
				"Emergency services active? " + services.stream().anyMatch(TransportService::isEmergencyService));

		System.out.println("\nEarliest available trips (sorted by time):");
		services.stream().flatMap(s -> s.getAvailableTrips().stream())
				.sorted(Comparator.comparing(Trip::getDepartureTime)).limit(5).forEach(System.out::println);
		...
		double dist = GeoUtils.calculateDistance(new Location(23.2599, 77.4126), new Location(23.1815, 77.4350));
		...
		List<Passenger> passengers = Arrays.asList(new Passenger("Abhishek", "P001", 45.0, "B101", true),
				new Passenger("Priya", "P002", 28.0, "M201", false),
				new Passenger("Rohan", "P003", 180.0, "T301", true), new Passenger("Sneha", "P004", 42.0, "B101", true),
				new Passenger("Vikram", "P005", 35.0, "B101", false));

		System.out.println("\nRevenue Summary by Route:");
		Map<String, Double> revenueByRoute = passengers.stream().collect(
				Collectors.groupingBy(Passenger::getRouteId, Collectors.summingDouble(Passenger::getFarePaid)));
		revenueByRoute
				.forEach((route, total) -> System.out.printf("Route %s --> Total revenue: Rs.%.2f%n", route, total));
